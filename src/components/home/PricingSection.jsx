    ...
    <section id="pricing" className="section-wrapper section-alt pricing-decision-section">
      <div className="container-wide">
        ...
        <div className="tabs-scroll-wrapper">
          <LightTabIndicator
            tabs={pricingPlans.map((plan) => ({ id: plan.id, label: plan.name }))}
            activeTab={selectedPlan}
            onChange={setSelectedPlan}
            variant="pill"
            className="pricing-scope-light-tabs"
          />
        </div>
        ...
        <div id="comparison" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '20px', maxWidth: '1120px', margin: '0 auto' }}>
          {pricingPlans.map((plan) => (
            <InteractiveTiltCard
              ...
              id={plan.id}
              key={plan.id}
              onPointerEnter={() => setSelectedPlan(plan.id)}
              className={`card-saas pricing-decision-card ${selectedPlan === plan.id ? 'is-selected' : ''}`}
              style={{
                display: 'flex',
                flexDirection: 'column',
                padding: '30px',
                borderColor: plan.highlight ? 'var(--oda-primary-light)' : undefined,
                scrollMarginTop: '110px',
                boxShadow: selectedPlan === plan.id ? '0 12px 35px rgba(37, 99, 235, 0.2)' : 'none',
              }}
            >
